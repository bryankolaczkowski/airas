...
if len(sys.argv) < 3:
    sys.stderr.write("usage: %s protein cortype\n" % sys.argv[0])
    sys.stderr.write("  where protein = CARD1 | DSRM1 | DSRM2 | DSRM3 | RD1\n")
    sys.stderr.write("    and cortype = all | res | gap \n")
    sys.exit(1)
...
results = {}
handle = open("../%s.compiledCorrects.csv" % protein, "r")
handle.readline()
for line in handle:
    ...
    cor     = data[base_index]
    corPP   = data[base_index + index_inc]
    SEcor   = data[base_index + (index_inc*2)]
    SEcorPP = data[base_index + (index_inc*3)]

    results[node][1][alignment] = (cor,corPP, SEcor,SEcorPP)
handle.close()
