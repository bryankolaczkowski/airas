...
for protein in proteins:
    outf = open("err_reports_02/%s.origErrors.csv" % protein, "w")
    ...
    for f in glob.glob("err_reports_01/%s.R*.*.SeqErrors_orig.csv" % protein):
        rep = f.split("/")[-1].split(".")[3]
        aln = f.split("/")[-1].split(".")[4]

        # read alignment length #
        fname = "../asr/%s/%s/airas_RESULTS_correct_anc_seqs_aligned.fasta" % (protein,rep)
        handle = open(fname, "r")
        handle.readline()
